...
control c_stats_ip(inout header_t hdr, inout ingress_metadata_t ig_md) {
	...
	RegisterAction<cntr_cur, bit<16>, bit<32>>(reg_ip_pkt_cnt) ract_pkt_cnt_1_read = {
		void apply(inout cntr_cur cntr, out bit<32> res) {
			res = cntr.cntr_1;
			cntr.cntr_0_old = hdr.anonarch.ip_pkt_cnt;
		}
	};
	...
	RegisterAction<cntr_cur, bit<16>, bit<32>>(reg_ip_mean) ract_mean_1_read = {
		void apply(inout cntr_cur cntr, out bit<32> res) {
			res = cntr.cntr_1;
			cntr.cntr_0_old = hdr.anonarch.ip_mean;
		}
	};
	...
	RegisterAction<_, bit<1>, bit<32>>(reg_ip_mean_sqr_0) ract_mean_sqr_0_calc = {
		void apply(inout bit<32> val, out bit<32> res) {
			val = mean_sqr_0.execute(hdr.anonarch.ip_mean);
			res = val;
		}
	};
	...
	*/
	...
	action pkt_cnt_0_incr() {
		hdr.anonarch.ip_pkt_cnt = ract_pkt_cnt_0_incr.execute(ig_md.stats_ip.hash_0);
	}
	...
	action res_0_calc() {
		ig_md.stats_ip.res_0 = ig_md.stats_ip.pkt_len - hdr.anonarch.ip_mean;
	}
	...
	action rshift_mean_0() {
		hdr.anonarch.ip_mean = ig_md.stats_ip.pkt_len;
	}

	action rshift_mean_1() {
		hdr.anonarch.ip_mean = ig_md.stats_ip.pkt_len >> 1;
	}

	action rshift_mean_2() {
		hdr.anonarch.ip_mean = ig_md.stats_ip.pkt_len >> 2;
	}

	action rshift_mean_3() {
		hdr.anonarch.ip_mean = ig_md.stats_ip.pkt_len >> 3;
	}

	action rshift_mean_4() {
		hdr.anonarch.ip_mean = ig_md.stats_ip.pkt_len >> 4;
	}

	action rshift_mean_5() {
		hdr.anonarch.ip_mean = ig_md.stats_ip.pkt_len >> 5;
	}

	action rshift_mean_6() {
		hdr.anonarch.ip_mean = ig_md.stats_ip.pkt_len >> 6;
	}

	action rshift_mean_7() {
		hdr.anonarch.ip_mean = ig_md.stats_ip.pkt_len >> 7;
	}

	action rshift_mean_8() {
		hdr.anonarch.ip_mean = ig_md.stats_ip.pkt_len >> 8;
	}

	action rshift_mean_9() {
		hdr.anonarch.ip_mean = ig_md.stats_ip.pkt_len >> 9;
	}

	action rshift_mean_10() {
		hdr.anonarch.ip_mean = ig_md.stats_ip.pkt_len >> 10;
	}

	action rshift_mean_11() {
		hdr.anonarch.ip_mean = ig_md.stats_ip.pkt_len >> 11;
	}

	action rshift_mean_12() {
		hdr.anonarch.ip_mean = ig_md.stats_ip.pkt_len >> 12;
	}

	action rshift_mean_13() {
		hdr.anonarch.ip_mean = ig_md.stats_ip.pkt_len >> 13;
	}

	action rshift_mean_14() {
		hdr.anonarch.ip_mean = ig_md.stats_ip.pkt_len >> 14;
	}

	action rshift_mean_15() {
		hdr.anonarch.ip_mean = ig_md.stats_ip.pkt_len >> 15;
	}

	action rshift_mean_16() {
		hdr.anonarch.ip_mean = ig_md.stats_ip.pkt_len >> 16;
	}

	action rshift_mean_17() {
		hdr.anonarch.ip_mean = ig_md.stats_ip.pkt_len >> 17;
	}

	action rshift_mean_18() {
		hdr.anonarch.ip_mean = ig_md.stats_ip.pkt_len >> 18;
	}

	action rshift_mean_19() {
		hdr.anonarch.ip_mean = ig_md.stats_ip.pkt_len >> 19;
	}

	action rshift_mean_20() {
		hdr.anonarch.ip_mean = ig_md.stats_ip.pkt_len >> 20;
	}
	...
	*/
	...
	action std_dev_0_calc() {
		hdr.anonarch.ip_std_dev = ract_std_dev_0_calc.execute(0);
	}
	...
	*/
	...
	action magnitude_calc() {
		hdr.anonarch.ip_magnitude = ract_magnitude_calc.execute(0);
	}
	*/
	...
	*/
	...
	/*
	action radius_calc() {
		hdr.anonarch.ip_radius = ract_radius_calc.execute(0);
	}
	*/

	action pkt_cnt_sum() {
		ig_md.stats_ip.pkt_cnt_1 = ig_md.stats_ip.pkt_cnt_1 + hdr.anonarch.ip_pkt_cnt;
	}

	action rshift_cov_0() {
		hdr.anonarch.ip_cov = ig_md.stats_ip.sum_res_prod;
	}

	action rshift_cov_1() {
		hdr.anonarch.ip_cov = ig_md.stats_ip.sum_res_prod >> 1;
	}

	action rshift_cov_2() {
		hdr.anonarch.ip_cov = ig_md.stats_ip.sum_res_prod >> 2;
	}

	action rshift_cov_3() {
		hdr.anonarch.ip_cov = ig_md.stats_ip.sum_res_prod >> 3;
	}

	action rshift_cov_4() {
		hdr.anonarch.ip_cov = ig_md.stats_ip.sum_res_prod >> 4;
	}

	action rshift_cov_5() {
		hdr.anonarch.ip_cov = ig_md.stats_ip.sum_res_prod >> 5;
	}

	action rshift_cov_6() {
		hdr.anonarch.ip_cov = ig_md.stats_ip.sum_res_prod >> 6;
	}

	action rshift_cov_7() {
		hdr.anonarch.ip_cov = ig_md.stats_ip.sum_res_prod >> 7;
	}

	action rshift_cov_8() {
		hdr.anonarch.ip_cov = ig_md.stats_ip.sum_res_prod >> 8;
	}

	action rshift_cov_9() {
		hdr.anonarch.ip_cov = ig_md.stats_ip.sum_res_prod >> 9;
	}

	action rshift_cov_10() {
		hdr.anonarch.ip_cov = ig_md.stats_ip.sum_res_prod >> 10;
	}

	action rshift_cov_11() {
		hdr.anonarch.ip_cov = ig_md.stats_ip.sum_res_prod >> 11;
	}

	action rshift_cov_12() {
		hdr.anonarch.ip_cov = ig_md.stats_ip.sum_res_prod >> 12;
	}

	action rshift_cov_13() {
		hdr.anonarch.ip_cov = ig_md.stats_ip.sum_res_prod >> 13;
	}

	action rshift_cov_14() {
		hdr.anonarch.ip_cov = ig_md.stats_ip.sum_res_prod >> 14;
	}

	action rshift_cov_15() {
		hdr.anonarch.ip_cov = ig_md.stats_ip.sum_res_prod >> 15;
	}

	action rshift_cov_16() {
		hdr.anonarch.ip_cov = ig_md.stats_ip.sum_res_prod >> 16;
	}

	action rshift_cov_17() {
		hdr.anonarch.ip_cov = ig_md.stats_ip.sum_res_prod >> 17;
	}

	action rshift_cov_18() {
		hdr.anonarch.ip_cov = ig_md.stats_ip.sum_res_prod >> 18;
	}

	action rshift_cov_19() {
		hdr.anonarch.ip_cov = ig_md.stats_ip.sum_res_prod >> 19;
	}

	action rshift_cov_20() {
		hdr.anonarch.ip_cov = ig_md.stats_ip.sum_res_prod >> 20;
	}

	action lshift_std_dev_prod_0() {
		ig_md.stats_ip.std_dev_prod = hdr.anonarch.ip_std_dev;
	}

	action lshift_std_dev_prod_1() {
		ig_md.stats_ip.std_dev_prod = hdr.anonarch.ip_std_dev << 1;
	}

	action lshift_std_dev_prod_2() {
		ig_md.stats_ip.std_dev_prod = hdr.anonarch.ip_std_dev << 2;
	}

	action lshift_std_dev_prod_3() {
		ig_md.stats_ip.std_dev_prod = hdr.anonarch.ip_std_dev << 3;
	}

	action lshift_std_dev_prod_4() {
		ig_md.stats_ip.std_dev_prod = hdr.anonarch.ip_std_dev << 4;
	}

	action lshift_std_dev_prod_5() {
		ig_md.stats_ip.std_dev_prod = hdr.anonarch.ip_std_dev << 5;
	}

	action lshift_std_dev_prod_6() {
		ig_md.stats_ip.std_dev_prod = hdr.anonarch.ip_std_dev << 6;
	}

	action lshift_std_dev_prod_7() {
		ig_md.stats_ip.std_dev_prod = hdr.anonarch.ip_std_dev << 7;
	}

	action lshift_std_dev_prod_8() {
		ig_md.stats_ip.std_dev_prod = hdr.anonarch.ip_std_dev << 8;
	}

	action lshift_std_dev_prod_9() {
		ig_md.stats_ip.std_dev_prod = hdr.anonarch.ip_std_dev << 9;
	}

	action lshift_std_dev_prod_10() {
		ig_md.stats_ip.std_dev_prod = hdr.anonarch.ip_std_dev << 10;
	}

	action lshift_std_dev_prod_11() {
		ig_md.stats_ip.std_dev_prod = hdr.anonarch.ip_std_dev << 11;
	}

	action rshift_pcc_0() {
		hdr.anonarch.ip_pcc = hdr.anonarch.ip_cov;
	}

	action rshift_pcc_1() {
		hdr.anonarch.ip_pcc = hdr.anonarch.ip_cov >> 1;
	}

	action rshift_pcc_2() {
		hdr.anonarch.ip_pcc = hdr.anonarch.ip_cov >> 2;
	}

	action rshift_pcc_3() {
		hdr.anonarch.ip_pcc = hdr.anonarch.ip_cov >> 3;
	}

	action rshift_pcc_4() {
		hdr.anonarch.ip_pcc = hdr.anonarch.ip_cov >> 4;
	}

	action rshift_pcc_5() {
		hdr.anonarch.ip_pcc = hdr.anonarch.ip_cov >> 5;
	}

	action rshift_pcc_6() {
		hdr.anonarch.ip_pcc = hdr.anonarch.ip_cov >> 6;
	}

	action rshift_pcc_7() {
		hdr.anonarch.ip_pcc = hdr.anonarch.ip_cov >> 7;
	}

	action rshift_pcc_8() {
		hdr.anonarch.ip_pcc = hdr.anonarch.ip_cov >> 8;
	}

	action rshift_pcc_9() {
		hdr.anonarch.ip_pcc = hdr.anonarch.ip_cov >> 9;
	}

	action rshift_pcc_10() {
		hdr.anonarch.ip_pcc = hdr.anonarch.ip_cov >> 10;
	}

	action rshift_pcc_11() {
		hdr.anonarch.ip_pcc = hdr.anonarch.ip_cov >> 11;
	}

	action rshift_pcc_12() {
		hdr.anonarch.ip_pcc = hdr.anonarch.ip_cov >> 12;
	}

	action rshift_pcc_13() {
		hdr.anonarch.ip_pcc = hdr.anonarch.ip_cov >> 13;
	}

	action rshift_pcc_14() {
		hdr.anonarch.ip_pcc = hdr.anonarch.ip_cov >> 14;
	}

	action rshift_pcc_15() {
		hdr.anonarch.ip_pcc = hdr.anonarch.ip_cov >> 15;
	}

	action rshift_pcc_16() {
		hdr.anonarch.ip_pcc = hdr.anonarch.ip_cov >> 16;
	}

	action rshift_pcc_17() {
		hdr.anonarch.ip_pcc = hdr.anonarch.ip_cov >> 17;
	}

	action rshift_pcc_18() {
		hdr.anonarch.ip_pcc = hdr.anonarch.ip_cov >> 18;
	}

	action rshift_pcc_19() {
		hdr.anonarch.ip_pcc = hdr.anonarch.ip_cov >> 19;
	}

	action rshift_pcc_20() {
		hdr.anonarch.ip_pcc = hdr.anonarch.ip_cov >> 20;
	}

	action rshift_pcc_21() {
		hdr.anonarch.ip_pcc = hdr.anonarch.ip_cov >> 21;
	}

	action rshift_pcc_22() {
		hdr.anonarch.ip_pcc = hdr.anonarch.ip_cov >> 22;
	}

	action rshift_pcc_23() {
		hdr.anonarch.ip_pcc = hdr.anonarch.ip_cov >> 23;
	}

	action rshift_pcc_24() {
		hdr.anonarch.ip_pcc = hdr.anonarch.ip_cov >> 24;
	}

	action rshift_pcc_25() {
		hdr.anonarch.ip_pcc = hdr.anonarch.ip_cov >> 25;
	}

	action rshift_pcc_26() {
		hdr.anonarch.ip_pcc = hdr.anonarch.ip_cov >> 26;
	}

	action rshift_pcc_27() {
		hdr.anonarch.ip_pcc = hdr.anonarch.ip_cov >> 27;
	}

	action rshift_pcc_28() {
		hdr.anonarch.ip_pcc = hdr.anonarch.ip_cov >> 28;
	}

	action rshift_pcc_29() {
		hdr.anonarch.ip_pcc = hdr.anonarch.ip_cov >> 29;
	}

	action rshift_pcc_30() {
		hdr.anonarch.ip_pcc = hdr.anonarch.ip_cov >> 30;
	}

	action rshift_pcc_31() {
		hdr.anonarch.ip_pcc = hdr.anonarch.ip_cov >> 31;
	}
	...
	table mean_0 {
		key = {
			hdr.anonarch.ip_pkt_cnt : ternary;
		}
		actions = {
			rshift_mean_0;
			rshift_mean_1;
	...
	apply {
		...
		mean_sqr_0.apply(hdr.anonarch.ip_mean, ig_md.stats_ip.mean_sqr_0);
		...
		magnitude.apply(ig_md.stats_ip.mean_sqr_0, hdr.anonarch.ip_magnitude);
		...
		std_dev_0.apply(ig_md.stats_ip.variance_0, hdr.anonarch.ip_std_dev);
		...
		// Radius calculation (math_unit).
		radius.apply(ig_md.stats_ip.variance_sqr_0, hdr.anonarch.ip_radius);
