...
control SwitchIngress_a(
    ...
    action modify_eg_port(PortId_t port) {
        ig_tm_md.ucast_egress_port = port;
        in_counter.count();
        hdr.anonarch.setValid();
        hdr.anonarch.decay = (bit<32>)ig_md.meta.decay_cntr;
    }

    action set_anonarch_mac_ip_src_a() {
        hdr.anonarch.mac_ip_src_pkt_cnt = ig_md.stats_mac_ip_src.pkt_cnt_0;
        hdr.anonarch.mac_ip_src_pkt_len = ig_md.stats_mac_ip_src.pkt_len;
        hdr.anonarch.mac_ip_src_ss = ig_md.stats_mac_ip_src.ss;
    }

    action set_anonarch_ip_src_a() {
        hdr.anonarch.ip_src_pkt_cnt = ig_md.stats_ip_src.pkt_cnt_0;
        hdr.anonarch.ip_src_pkt_len = ig_md.stats_ip_src.pkt_len;
        hdr.anonarch.ip_src_ss = ig_md.stats_ip_src.ss;
        /* hdr.anonarch.ip_src_ts_diff = ig_md.stats_ip_src.ts_diff; */
    }

    action set_anonarch_ip_a() {
        hdr.anonarch.ip_pkt_cnt = ig_md.stats_ip.pkt_cnt_0;
        hdr.anonarch.ip_pkt_cnt_1 = ig_md.stats_ip.pkt_cnt_1;
        hdr.anonarch.ip_ss_0 = ig_md.stats_ip.ss_0;
        hdr.anonarch.ip_ss_1 = ig_md.stats_ip.ss_1;
        hdr.anonarch.ip_mean_0 = ig_md.stats_ip.mean_0;
        hdr.anonarch.ip_mean_1 = ig_md.stats_ip.mean_1;
        hdr.anonarch.ip_sum_res_prod_cov = ig_md.stats_ip.sum_res_prod;
    }

    action set_anonarch_five_t_a() {
        hdr.anonarch.five_t_pkt_cnt = ig_md.stats_five_t.pkt_cnt_0;
        hdr.anonarch.five_t_pkt_cnt_1 = ig_md.stats_five_t.pkt_cnt_1;
        hdr.anonarch.five_t_ss_0 = ig_md.stats_five_t.ss_0;
        hdr.anonarch.five_t_ss_1 = ig_md.stats_five_t.ss_1;
        hdr.anonarch.five_t_mean_0 = ig_md.stats_five_t.mean_0;
        hdr.anonarch.five_t_mean_1 = ig_md.stats_five_t.mean_1;
        hdr.anonarch.five_t_sum_res_prod_cov = ig_md.stats_five_t.sum_res_prod;
    }
    ...
    apply {
        if (hdr.ipv4.isValid()) {
            ...
            if (ig_md.meta.recirc_toggle == 1) {
                set_anonarch_mac_ip_src_a();
                set_anonarch_ip_src_a();
                set_anonarch_ip_a();
                set_anonarch_five_t_a();
            }
        }
    }
}
